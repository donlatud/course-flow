...
const emits = defineEmits<{ (e: "update:modelValue", payload: Date | undefined): void }>()
const date = useVModel(props, "modelValue", emits, { passive: true })

const maxSelectableDate = computed(() => today(getLocalTimeZone()).subtract({ years: 12 }))
const minSelectableDate = computed(() => today(getLocalTimeZone()).subtract({ years: 100 }))
...
const isDateDisabled = (dateVal: DateValue) => {
  ...
  return jsDate > subYears(startOfDay(todayDate), 12) || jsDate < subYears(startOfDay(todayDate), 100)
}
...
const calendarValue = computed<DateValue | undefined>(() => {
  ...
  return new CalendarDate(date.value.getFullYear(), date.value.getMonth() + 1, date.value.getDate()) as any
})

const formattedDate = computed(() => date.value ? format(date.value, "dd/MM/yyyy") : "")

watch(date, (newVal) => {
  if (newVal) calendarPlaceholder.value = new CalendarDate(newVal.getFullYear(), newVal.getMonth() + 1, newVal.getDate()) as any
}, { immediate: true })
</script>

<template>
  <div :class="cn('flex flex-col gap-1 w-full', props.class)">
    <label v-if="label" class="text-body3 font-medium text-gray-800">{{ label }}</label>
      ...
      <PopoverTrigger as-child>
        <Button
          ...
          :class="cn(
            ...
            error && 'border-(--color-purple)! ring-2! ring-(--color-purple)/20!',
            disabled && 'bg-gray-200! border-gray-400! text-gray-600! cursor-not-allowed!'
          )"
        >
          <span class="text-body2 text-gray-900">{{ formattedDate || placeholder || 'dd/MM/yyyy' }}</span>
    ...
    <p v-if="error && errorMessage" class="text-body4 text-(--color-purple) mt-0.5">{{ errorMessage }}</p>
  </div>
</template>